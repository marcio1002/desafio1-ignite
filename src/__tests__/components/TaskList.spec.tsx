...
describe("App Page", () => {
  ...
  it("should be able to update the task", () => {
    ...
    fireEvent.change(taskInput, {
      target: {
        value: "Beber muito café!",
      },
    });
    fireEvent.click(addTaskButton);

    const taskText = screen.getByText("Beber muito café!");

    expect(taskText).toHaveTextContent("Beber muito café!");
    ...
    fireEvent.change(editTaskInput, {
      target: {
        value: "Beber muita água!",
      },
    });
    fireEvent.blur(editTaskInput);

    const updateTaskText = screen.getByText("Beber muita água!");

    expect(updateTaskText).toHaveTextContent("Beber muita água!");
  });
});
